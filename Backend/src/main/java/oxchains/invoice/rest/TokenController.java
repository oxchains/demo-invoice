...
public class TokenController {
    ...
    public TokenController(@Autowired UserRepo userRepo, @Autowired UserTokenRepo userTokenRepo, @Autowired JwtService jwtService) {
    ...
    public RestResp enroll(@RequestBody NameAndPass user) {
        return userRepo
          ...
          .map(u -> {
              UserToken userToken = new UserToken(u, jwtService.generate(u, user.isBiz()));
