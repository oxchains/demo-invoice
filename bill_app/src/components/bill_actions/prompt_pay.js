...
import { prompt_pay } from '../../actions/bill'
...
class PromptPay extends Component {
  ...
  handleFormSubmit({id, manipulator, action}) {
    if(id && manipulator) {
      ...
      this.props.prompt_pay({id, manipulator, action}, err => {
        ...
        <form className="form-horizontal" onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
          ...
          <div className="form-group">
            ...
            <div className="col-sm-10">
              ...
              <label className="margin-r-5"><Field name="action" component="input" type="radio" value="-1"/> 拒绝付款</label>
              <label className="margin-r-5"><Field name="action" component="input" type="radio" value="1"/> 确认付款</label>
            </div>
          </div>
          <div className="row">
...
const selector = formValueSelector('PromptPayForm');

const reduxPromptPayForm = reduxForm({
  form: 'PromptPayForm',
  validate
})(PromptPay);

export default connect(state => { return {action:selector(state, 'action')} }, { prompt_pay })(reduxPromptPayForm);