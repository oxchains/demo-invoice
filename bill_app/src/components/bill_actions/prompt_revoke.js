...
import { prompt_revoke } from '../../actions/bill'

class PromptRevoke extends Component {
  constructor(props) {
    super(props);
  ...
  handleFormSubmit({ id, manipulator, action }) {
    if(id && manipulator) {
      ...
      this.props.prompt_revoke({ id, manipulator, action }, err => {
...
const reduxPromptRevokeForm = reduxForm({
  form: 'PromptRevokeForm',
  validate
})(PromptRevoke);

export default connect(null, { prompt_revoke })(reduxPromptRevokeForm);