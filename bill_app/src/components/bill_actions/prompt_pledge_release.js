...
import { prompt_pledge_release } from '../../actions/bill'

class PromptPledgeRelease extends Component {
  constructor(props) {
    super(props);
  ...
  handleFormSubmit({id, manipulator, action, pledgee}) {
    if(id && manipulator) {
      ...
      this.props.prompt_pledge_release({id, manipulator, action, pledgee}, err => {
...
const selector = formValueSelector('PromptPledgeReleaseForm');

const reduxPromptPledgeReleaseForm = reduxForm({
  form: 'PromptPledgeReleaseForm',
  validate
})(PromptPledgeRelease);

export default connect(state => { return {action:selector(state, 'action')} }, { prompt_pledge_release })(reduxPromptPledgeReleaseForm);