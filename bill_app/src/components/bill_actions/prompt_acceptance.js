...
import { prompt_acceptance } from '../../actions/bill'

class PromptAcceptance extends Component {
  constructor(props) {
    super(props);
  ...
  handleFormSubmit({ id, manipulator, action }) {
    if(id && manipulator) {
      ...
      this.props.prompt_acceptance({ id, manipulator, action }, err => {
...
const reduxPromptAcceptanceForm = reduxForm({
  form: 'PromptAcceptanceForm',
  validate
})(PromptAcceptance);

export default connect(null, { prompt_acceptance })(reduxPromptAcceptanceForm);