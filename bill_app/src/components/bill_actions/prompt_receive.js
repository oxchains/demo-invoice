...
import { prompt_receive } from '../../actions/bill'

class PromptReceive extends Component {
  constructor(props) {
    super(props);
  ...
  handleFormSubmit({ id, manipulator, action }) {
    if(id && manipulator) {
      ...
      this.props.prompt_receive({ id, manipulator, action }, err => {
...
const reduxPromptReceiveForm = reduxForm({
  form: 'PromptReceiveForm',
  validate
})(PromptReceive);

export default connect(null, { prompt_receive })(reduxPromptReceiveForm);