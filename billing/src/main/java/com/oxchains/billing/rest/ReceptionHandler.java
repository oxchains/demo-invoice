...
public class ReceptionHandler extends ChaincodeUriBuilder {
  ...
  public ReceptionHandler(@Autowired WebClient client, @Autowired @Qualifier("fabric.uri") UriBuilder uriBuilder) {
  ...
  public Mono<ServerResponse> create(ServerRequest request) {
    return request.bodyToMono(PresentAction.class)
        .flatMap(receiveAction -> client.post().uri(buildUri(args(BILL_RECEIVE, receiveAction)))
  ...
  public Mono<ServerResponse> get(ServerRequest request) {
    ...
    return client.post().uri(buildUri(args(GET_RECEPTION, billId)))
        .accept(APPLICATION_JSON_UTF8).exchange()
        .filter(clientResponse -> clientResponse.statusCode().is2xxSuccessful())
        .flatMap(clientResponse -> Mono.just(toServerResponse(clientResponse)))
        .switchIfEmpty(noContent().build());
